...
trait CDFContextLogOps: CDFContextLogSize {
  ...
  fn push(log: &mut CDFContextLogBase, cdf: &[u16]) {
    ...
    unsafe {
      ...
      debug_assert!(len + Self::CDF_LEN_MAX + 1 <= log.data.capacity());
...
impl CDFContextLog {
  ...
  fn checkpoint(&self) -> (usize, usize) {
    (self.small.0.data.len(), self.large.0.data.len())
  }
  #[inline(always)]
  ...
  #[inline(always)]
  pub fn rollback(&mut self, fc: &mut CDFContext, checkpoint: (usize, usize)) {
    let (small, large) = checkpoint;
    CDFContextLogSmall::rollback(&mut self.small.0, fc, small);
    CDFContextLogLarge::rollback(&mut self.large.0, fc, large);
  }
