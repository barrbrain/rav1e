...
impl Plane {
  ...
  pub fn new(width: usize, height: usize, xdec: usize, ydec: usize, xpad: usize, ypad: usize) -> Plane {
    let xorigin = xpad.align_power_of_two(Plane::STRIDE_ALIGNMENT_LOG2 - 1) as isize;
    let yorigin = ypad as isize;
    let stride = (xorigin as usize + width + xpad).align_power_of_two(Plane::STRIDE_ALIGNMENT_LOG2 - 1);
  ...
  pub fn window(&self, x: usize, y: usize) -> Plane {
    ...
    let width = 32 >> xdec;
    let height = 32 >> ydec;
    let stride = (xpad + width + xpad).align_power_of_two(Plane::STRIDE_ALIGNMENT_LOG2 - 1);
    let alloc_height = ypad + height + ypad;
    let mut data = vec![128u16; stride * alloc_height];
    for (d, s) in data.chunks_mut(stride).zip(self.data[(y * self.cfg.stride)..].chunks(self.cfg.stride)) {
...
impl<'a> PlaneSlice<'a> {
  pub fn as_slice(&'a self) -> &'a [u16] {
    ...
    let base = (self.y + self.plane.cfg.yorigin) as usize * stride + (self.x + self.plane.cfg.xorigin) as usize;
    &self.plane.data[base..]
  }

  pub fn as_slice_clamped(&'a self) -> &'a [u16] {
    let stride = self.plane.cfg.stride;
    let y = (self.y.min(self.plane.cfg.height as isize) + self.plane.cfg.yorigin).max(0) as usize;
    let x = (self.x.min(self.plane.cfg.width as isize) + self.plane.cfg.xorigin).max(0) as usize;
