...
macro_rules! impl_itx_fns {
  ...
  ([$([$(($ENUM:pat, $TYPE1:ident, $TYPE2:ident)),*]),*], $W:literal, $H:literal) => {
    paste::item! {
      ...
      extern {
        $(
          $(
            ...
            fn [<rav1e_inv_txfm_add_ $TYPE2 _$TYPE1 _$W x $H _avx2>](
              dst: *mut u8, dst_stride: libc::ptrdiff_t, coeff: *const i16,
              eob: i32
            );
            fn [<rav1e_inv_txfm_add_ $TYPE2 _$TYPE1 _$W x $H _ssse3>](
              dst: *mut u8, dst_stride: libc::ptrdiff_t, coeff: *const i16,
              eob: i32
            );
          )*
          ...
          match tx_type {
            $(
              $(
                ...
                a if a == $ENUM => {
                  // Note: type1 and type2 are flipped
                  [<rav1e_inv_txfm_add_$TYPE2 _$TYPE1 _$W x $H _avx2>]
                },
              )*
            )*
            _ => unreachable!()
          ...
          match tx_type {
            $(
              $(
                ...
                a if a == $ENUM => {
                  // Note: type1 and type2 are flipped
                  [<rav1e_inv_txfm_add_$TYPE2 _$TYPE1 _$W x $H _ssse3>]
                },
              )*
            )*
            _ => unreachable!()
  ...
  ($TYPES_VALID:tt, [$(($W:literal, $H:literal)),*]) => {
    $(
      impl_itx_fns!($TYPES_VALID, $W, $H);
    )*
  };
  ...
  ($TYPES64:tt, $DIMS64:tt, $TYPES32:tt, $DIMS32:tt, $TYPES16:tt, $DIMS16:tt,
   $TYPES84:tt, $DIMS84:tt) => {
    ...
    impl_itx_fns!([$TYPES64], $DIMS64);
    impl_itx_fns!([$TYPES64, $TYPES32], $DIMS32);
    impl_itx_fns!([$TYPES64, $TYPES32, $TYPES16], $DIMS16);
    impl_itx_fns!(
