...
// TODO: Mark const once we can use "if"s in a const function.
/* TODO vfr:
fn bexp64_q24(log_scale: i64) -> i32 {
  if log_scale < q57(8) {
    ::std::cmp::min(bexp64(log_scale + q57(24)), ::std::i32::MAX as i64) as i32
  }
  else {
    ::std::i32::MAX
  }
...
fn warp_alpha(alpha: i32) -> i32 {
  let i = ::std::cmp::min(alpha * 36 >> 24, 16);
...
fn iir_bessel2_get_parameters(delay: i32) -> (i32, i32, i32) {
  ...
  let warp = std::cmp::max(1, warp_alpha(alpha)) as i64;
  // k1 is 9.12 (6.12?)
  let k1 = 3 * warp;
  // k2 is 16.24 (11.24?)
  let k2 = k1 * warp;
...
impl QuantizerParameters {
  fn new_from_log_q(
    ...
    println!("log_base_q: {}", (log_base_q as f64) / (q57(1) as f64));
    println!("log_target_q: {}", (log_target_q as f64) / (q57(1) as f64));
    println!("dc_qi: {}", select_dc_qi(quantizer, bit_depth as usize));
    println!("ac_qi: {}", select_ac_qi(quantizer, bit_depth as usize));
    QuantizerParameters {
      log_base_q,
      log_target_q,
...
impl RCState {
  pub fn new(
    ...
    let reservoir_frame_delay = ::std::cmp::max(
      12,
      ::std::cmp::min(max_key_frame_interval * 3 >> 1, 256)
    );
    ...
    let bits_per_frame = ::std::cmp::max(
      32,
      ::std::cmp::min(
    ...
    RCState {
      ...
      // TODO vfr: vfrfilter: IIRBessel2::new(4, bexp64_q24(q57(0))),
  ...
  // Scales the number of frames by the expected VFR rate.
  // TODO: We should scale something with more precision than the frame counts.
  /* TODO vfr:
  fn scale_vfr(&self, nframes: i32) -> i32 {
    if self.log_vfr_scale != q57(0) {
      let vfr_scale = bexp64(self.log_vfr_scale + q57(8));
      if vfr_scale < (nframes << 8) as i64 {
        let vfr_scale32 = vfr_scale as i32;
        if vfr_scale32 > 0 {
          return ((nframes << 8) + vfr_scale32 - 1)/vfr_scale32;
        }
      }
      else {
        return (nframes != 0) as i32;
      }
    }
    nframes
  }*/
    ...
    if self.target_bitrate <= 0 {
      ...
      match self.twopass_state {
        ...
        _ => {
          ...
          let mut log_qhi = blog64(ac_q(
            self.maybe_ac_qi_max.unwrap_or(255),
            0,
            bit_depth as usize
          ) as i64)
            - q57(QSCALE + bit_depth - 8);
          ...
          if self.cap_overflow {
            ...
            if log_scale_pixels - log_q_exp < log_soft_limit {
              ...
              log_q_exp += ((log_scale_pixels - log_soft_limit - log_q_exp)
                >> 32)
                * (::std::cmp::min(margin, soft_limit) << 32)
                / margin;
