...
const MQP_Q12: &[i32; FRAME_NSUBTYPES] = &[
  ...
  //  stable.
  (1.000732421875 * (1 << 12) as f64) as i32,
  (1.0 * (1 << 12) as f64) as i32,
  (1.002197265625 * (1 << 12) as f64) as i32,
  (1.0029296875 * (1 << 12) as f64) as i32,
];

// These values were found by linear regression with the MSU FullHD comparison
//  2021 sequences.
const DQP_Q57: &[i64; FRAME_NSUBTYPES] = &[
  (-0.20848870152399065 * (1i64 << 57) as f64) as i64,
  (0.0 * (1i64 << 57) as f64) as i64,
  (0.4398106283604967 * (1i64 << 57) as f64) as i64,
  (0.6900178441263356 * (1i64 << 57) as f64) as i64,
];
