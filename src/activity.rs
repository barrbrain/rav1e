...
impl ActivityMask {
  ...
  pub fn from_plane<T: Pixel>(
    luma_plane: &Plane<T>, width: usize, height: usize,
  ) -> ActivityMask {
    ...
    for y in 0..h_in_imp_b {
      for x in 0..w_in_imp_b {
        let block_rect = Area::Rect {
          x: (x << 3) as isize,
          y: (y << 3) as isize,
          width: (width - (x << 3)).min(8),
          height: (height - (y << 3)).min(8),
        };
...
fn variance_8x8<T: Pixel>(src: &PlaneRegion<'_, T>) -> u32 {
  ...
  if src.rect().width != 8 || src.rect().height != 8 {
    return variance_8x8_padded(src);
  }
...
#[inline(never)]
fn variance_8x8_padded<T: Pixel>(src: &PlaneRegion<'_, T>) -> u32 {
  debug_assert!(src.plane_cfg.xdec == 0);
  debug_assert!(src.plane_cfg.ydec == 0);
  ...
  let height = src.rect().height;

  let mut row = [T::cast_from(0); 8];
  for j in 0..8 {
    if j < height {
