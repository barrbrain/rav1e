  ...
  fn pred_dc(output: &mut [T], stride: usize, above: &[T], left: &[T]) {
    #[cfg(all(target_arch = "x86_64", not(windows)))]
    {
      if size_of::<T>() == 1 && is_x86_feature_detected!("avx2") {
        let mut topleft: AlignedArray<[T; 257]> = UninitializedAlignedArray();
        topleft.array[129..Self::W + 129].copy_from_slice(&above[..Self::W]);
        topleft.array[128-Self::H..128].copy_from_slice(&left[..Self::H]);
        return unsafe {
          rav1e_ipred_dc_avx2(
            output.as_mut_ptr() as *mut _,
            stride as libc::ptrdiff_t,
            topleft.array.as_ptr().offset(128) as *const _,
