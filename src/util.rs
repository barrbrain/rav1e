...
pub static LOG2_U8: [u8; 128] = [
  0, 3, 6, 9, 11, 14, 17, 20, 22, 25, 28, 30, 33, 36, 38, 41, 44, 46, 49, 51,
  54, 56, 59, 61, 63, 66, 68, 71, 73, 75, 78, 80, 82, 85, 87, 89, 92, 94, 96,
  98, 100, 103, 105, 107, 109, 111, 113, 116, 118, 120, 122, 124, 126, 128,
  130, 132, 134, 136, 138, 140, 142, 144, 146, 148, 150, 152, 154, 155, 157,
  159, 161, 163, 165, 167, 169, 170, 172, 174, 176, 178, 179, 181, 183, 185,
  186, 188, 190, 192, 193, 195, 197, 198, 200, 202, 203, 205, 207, 208, 210,
  212, 213, 215, 216, 218, 220, 221, 223, 224, 226, 228, 229, 231, 232, 234,
  235, 237, 238, 240, 241, 243, 244, 246, 247, 249, 250, 252, 253, 255,
];

pub fn fast_log2_q8(x: u64) -> i32 {
  if x <= 0 {
    return 0;
  }
  let log_x = x.ilog();
  let extra =
    LOG2_U8[if log_x >= 8 { x >> (log_x - 8) } else { x << (8 - log_x) }
      as usize - 128];
  (log_x - 1 << 8) as i32 | extra as i32
}

#[test]
fn fast_log2_q8_test() {
  for n in 2..std::u16::MAX as u64 {
    assert!(fast_log2_q8(n) as f64 / (n as f64).log2() > 255.5_f64);
    assert!(fast_log2_q8(n) as f64 / (n as f64).log2() < 256.5_f64);
  }
}
