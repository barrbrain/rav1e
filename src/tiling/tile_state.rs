...
mod test {
  use super::*;
  use crate::encoder::test::create_frame;

  #[test]
  fn check_tile_activity() {
    let (width, height) = (128, 128);
    let (frame, fi) = create_frame(width, height);
    let mut fs = FrameState::new_with_frame(&fi, std::sync::Arc::new(frame));
    fs.compute_activity_mask();
    let sbo = SuperBlockOffset {
      x: 0,
      y: 0,
    };
    let tsm = TileStateMut::new(&mut fs, sbo, 6, width, height);
    assert_eq!(tsm.get_activity(), 65535);
  }
}
